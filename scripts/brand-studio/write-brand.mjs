...
export function validateBrandPayload(body) {
	...
	const logo = assetPath(b.logo ?? '/images/hunt-showdown-cheats-logo.webp');
	const logoRaster = assetPath(b.logoRaster ?? '/images/hunt-showdown-cheats-logo.png');
	const defaultOgImage = assetPath(b.defaultOgImage ?? '/images/hunt-showdown-cheats-combat.webp');
	const heroImage = assetPath(b.heroImage ?? '/images/hunt-showdown-hero-banner.webp');
...
function extractionBlock(src, name) {
...
export function readBrandForStudio() {
	...
	const keywords = listMatch
		...
		: [];
	const seoBlock = extractionBlock(src, 'seo');
	const copyBlock = extractionBlock(src, 'copy');
	const sitemapBlock = extractionBlock(src, 'sitemap');
	const themeBlock = extractionBlock(src, 'theme');
	/** @type {Record<string, string>} */
	const seo = {};
